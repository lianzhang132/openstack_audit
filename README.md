...
- 从当前平台资源中选择镜像、规格、网络和启动卷创建云主机
- 支持镜像启动和已有启动卷启动
- 支持预创建 Neutron 端口，并可按需禁用端口安全
- 管理内部项目归属、用途、申请人和长期使用状态
- 根据到期时间自动将云主机加入待回收队列
- 独立管理虚机与附属资源的回收周期、延期和长期保留
- 记录创建、同步、修改、延期、回收和失败清理操作
- 禁止通过应用删除审计记录
- 将云主机与审计信息导出为 Excel
- 从 Excel 导入历史记录
...
- 创建云主机时会把创建参数、启动方式、端口安全设置和资源 ID 写入审计记录。
- 如果端口已创建但云主机创建失败，会保留一条待清理记录，避免端口成为无审计资源。
...
6. 首次使用“创建虚机”前，先在非生产项目中验证镜像启动、卷启动、端口安全关闭和回收流程。
...
- 镜像启动创建云主机，并审计禁用端口安全的专用端口
- 卷启动创建云主机，并登记启动卷附属资源
- 端口创建成功但虚机创建失败时保留可回收审计记录
- 同步列表失败时不误判云主机已删除
- 单台确认失败时不推进删除状态
- 云主机回收失败时保持原状态
- 附属资源部分失败时保持待回收状态
